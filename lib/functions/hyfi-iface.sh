...
local DNI_traffic_separation_enabled=0
local DNI_traffic_separation_active=0
local iface_config
...
__hyfi_get_wlan_ifaces() {
	...
	config_get network "$config" network
	config_get disabled "$config" disabled '0'
	config_get hyd_unmanaged "$config" hyd_unmanaged '0'

	if [ -n "$iface" -a "$2" = "$network" -a "$disabled" -eq 0 -a "$hyd_unmanaged" -eq 0 ]; then
...
__hyfi_get_ether_ifaces() {
	...
	if [ "$2" = "$config" ]; then
		...
		for iface in $ifnames; do
			...
			if [ "$DNI_traffic_separation_enabled" -gt 0 ] && \
				[ "$DNI_traffic_separation_active" -gt 0 ]; then
					if __hyfi_is_vlan_iface $iface; then
						if __hyfi_is_device_wlan $iface; then
							continue
						fi
...
__hyfi_iterate_wlan_ifaces() {
	...
	if [ -n "$iface" -a "$backhaul_network" = "$network" -a "$disabled" -eq 0 ] ||  
           [ -n "$iface" -a "$DNI_traffic_separation_enabled" -gt 0 -a "$disabled" -eq 0 ]; then
...
hyfi_get_wlan_vlan_ifaces() {
	...
	config_get DNI_traffic_separation_enabled repacd DNI_TrafficSeparationEnabled '0'
	config_get DNI_traffic_separation_active repacd DNI_TrafficSeparationActive '0'
	if [ "$DNI_traffic_separation_enabled" -gt 0 ] && \
		[ "$DNI_traffic_separation_active" -gt 0 ]; then
		config_load network
		config_foreach __hyfi_get_wlan_vlan_ifaces interface $ieee1905managed
