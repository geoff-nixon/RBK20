...
elif [ "$orbi_type" = "Base" ]; then
    #Check triband related setting
    wl5g_bh_ap_athx=`config get wl5g_2nd_NORMAL_AP`
    triband_mode=`config get triband_mode`
    printf " Checking triband related setting, current triband mode is $triband_mode\n"

    if [ "$triband_mode" = "1" ]; then
        DEFAULT_RPS_CPUS=d
    elif [ "$triband_mode" = "0" ]; then
        DEFAULT_RPS_CPUS=0
    fi

    wl5g_triband_rps_cpus=`cat /sys/class/net/$wl5g_bh_ap_athx/queues/rx-0/rps_cpus`
    show_result "$wl5g_bh_ap_athx.rps_cpus" "$wl5g_triband_rps_cpus" "$DEFAULT_RPS_CPUS"
fi
