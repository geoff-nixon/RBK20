...
WFIMON_5G_DOWN_LOG='/tmp/repacd_5G_down_log'
...
local rssi_num=0 rssi_filename= rssi24g_filename=
...
local rate_num=0 rate_min=0 rate_max=0 rate_pref2G=0 rate_to_CAP_filename=
local rssi_counter=0
local is_24G_down_by_independent_channel=0
local rssi_idle_time=0
local new_bestap new_bestap_otherband bestap_filename=''
local bssid_resolve_state nolink_detected_5g=0
local curr_root_distance=$RE_ROOT_AP_DISTANCE_INVALID
local ignore_24g_assoc=0
local my_uplink_rate_filename=
local MoveFromCAP_snr_threshold CAP_snr_filename=
local ARLO_VLAN_2g_sta=
local ARLO_VLAN_5g_sta=
...
local CAP_snr_threshold MoveFromCAP_snr_hysteresis
...
__repacd_stop_ping() {
    if [ "$ping_running" -gt 0 ]; then
        local child_id=$(cat /tmp/.repacd_ping)
        kill "$child_id"
        ping_running=0
    ...
    if [ -n "$rssi24g_filename" ]; then
        # Clean up the temporary file
        rm -f $rssi24g_filename
        rssi24g_filename=
    fi
...
#        associate_state=1
        ...
        [ "x`/bin/config get enable_arlo_function`" = "x1" ] && {
            if [ "$force_down_5g" -gt 0 ]; then
                ifconfig $ARLO_VLAN_5g_sta down
                ifconfig $ARLO_VLAN_2g_sta up
            else
                ifconfig $ARLO_VLAN_5g_sta up
                ifconfig $ARLO_VLAN_2g_sta down
            fi
        }
...
__repacd_wifimon_measure_rate() {
    ...
    rssi=`iwconfig $sta_iface_5g | grep 'Signal level' | awk -F'=' '{print $3}' | awk '{print $1}'`
    if [ $rssi -lt -250 ]; then
        __repacd_wifimon_debug "iwconfig rssi overflow"
        rssi=0
    fi
    ...
    if __repacd_wifimon_is_assoc $sta_iface_24g; then
        rssi_24g=`iwconfig $sta_iface_24g | grep 'Signal level' | awk -F'=' '{print $3}' | awk '{print $1}'`
    fi
    ...
    if [ -n "$rate_to_CAP" ]; then
		...
		rssi24g_filename=`mktemp /tmp/repacd-rssi24g.XXXXXX`
                if [ -n "$sta_iface_24g" ]; then
		...
		echo $rssi_24g >> $rssi24g_filename
            ...
            local rssi_median rate_to_CAP_median my_uplink_rate_median CAP_snr_median bestap_bssid_list rssi24g_median restarted=0 config_success=0
	    ...
	    __repacd_wifimon_compute_median $rssi24g_filename $rate_num rssi24g_median
            __repacd_wifimon_debug "Median RSSI = $rssi_median CAP SNR = $CAP_snr_median Rate to CAP = $rate_to_CAP_median Mbps"

            if __repacd_wifimon_is_assoc $sta_iface_5g; then
                echo "5G:$rssi_median " > /tmp/backhaul_rssi
            elif __repacd_wifimon_is_assoc $sta_iface_24g; then
                echo "2G:$rssi24g_median " > /tmp/backhaul_rssi
            fi

            force_daisy=$(/bin/config get force_daisy)
            if [ "$force_daisy" -eq "1" ]; then                                         
                CAP_snr_threshold=0
            fi   
...
__repacd_wifimon_measure_link() {
    ...
    if [ "$measuring_cnt" -gt "$measuring_attempts" ]; then
        if [ -n "$sta_iface_24g" ] &&
               __repacd_wifimon_is_assoc $sta_iface_24g ; then
            __repacd_wifimon_bring_iface_down $sta_iface_5g
            echo "Bring 5G down by over measuring attempts times" >  WFIMON_5G_DOWN_LOG
            measuring_cnt=0
            eval "$2=$WIFIMON_STATE_RE_MOVE_CLOSER"
            return
        fi
    ...
    if __repacd_wifimon_is_assoc $sta_iface_5g; then
        ...
        rssi_far=$rssi_far5g
        rssi=`iwconfig $sta_iface_5g | grep 'Signal level' | awk -F'=' '{print $3}' | awk '{print $1}'`
        current_nf=`iwpriv $sta_iface_5g get_channf | awk -F':' '{print $2}'`
        rssi=$((rssi + 110 + current_nf ))
    ...
    if [ "$is_independent_channel_selection_enable" -gt 0 -a "rssi24G_used" -eq 0 ]; then
        __repacd_wifimon_independent_channel_check $rssi
            ...
            if [ "$device_type" = 'RE' ]; then
                ...
                if [ "$rssi_median" -le "$rssi_pref2G" ] &&
                    ...
                    if [ -n "$sta_iface_24g" ] &&
                           __repacd_wifimon_is_assoc $sta_iface_24g ; then
                        __repacd_wifimon_bring_iface_down $sta_iface_5g
                        echo "Bring 5G down by rssi too weak" >  WFIMON_5G_DOWN_LOG
                    fi
                fi
...
__repacd_wifimon_is_timeout() {
    ...
    __repacd_wifimon_debug "repacd check timeout elapsed:$elapsed_time duration:$duration"
...
__repacd_wifimon_is_sta_iface() {
    ...
    config_get vid "$config" vid_ITEM1
...
__repacd_wifimon_evaluate_backhaul_link() {
    ...
    if [ "$force_down_5g" -gt 0 ] ; then
        ...
        if  __repacd_wifimon_is_timeout $force_down_5g_timestamp $backhaul_eval_time; then
            ...
            __repacd_wifimon_get_current_best_ap $sta_iface_5g bestap_median bestap_otherband_median
            __repacd_wifimon_config_best_ap $sta_iface_5g $sta_iface_5g_config_name restart_required
...
__repacd_wifimon_get_current_phyrate_in_mbps() {
    ...
    if [ $phyrate_unit = "Gb/s" ]; then
       phyrate_in_mbps=$(echo $phyrate_in_mbps${padding}} | sed 's/\.//g' | cut -c1-4)
    else
...
__repacd_wifimon_check_and_config_bssid() {
    ...
    if __repacd_wifimon_is_peer_bssid_set $sta_iface_config_name; then
        ...
        if [ "$orbi_project" = "Orbimini" ]; then
            /bin/config set wla_ul_bssid="$new_bssid"
        else
            /bin/config set wla_2nd_ul_bssid="$new_bssid"
        fi
            /bin/config commit
        elif [ "$sta_iface" = "$sta_iface_24g" ]; then
            /bin/config set wlg_ul_bssid="$new_bssid"
            /bin/config commit
        fi
...
__repacd_wifimon_get_current_best_ap() {
    ...
    force_daisy=$(/bin/config get force_daisy)                                  
    if [ "$force_daisy" -eq "1" ]; then                                         
        best_bssid=$(/bin/config get force5g_mac)                               
        best_otherband_bssid=$(/bin/config get force2g_mac)                     
    fi   

    __repacd_wifimon_debug "best_bssid:$best_bssid  best_otherband_bssid:$best_otherband_bssid"
...
__repacd_wifimon_set_otherband_bssid() {
    ...
    __repacd_wifimon_debug "Set backhaul AP OtherBand BSSID"
                        ...
                        if [ -n "$sta_iface_24g" ] && [ -n "$sta_iface_5g" ]; then
                            if [ "$force_down_24g" -eq 0 -a "$force_down_5g" -eq 0 ]; then
                                if __repacd_wifimon_is_assoc $sta_iface_5g; then
                                    ...
                                    __repacd_wifimon_bring_iface_down $sta_iface_5g
                                    echo "Bring 5G down by BSSID timeout" >  WFIMON_5G_DOWN_LOG
                                    __repacd_wifimon_get_timestamp assoc_start_time
                                else
